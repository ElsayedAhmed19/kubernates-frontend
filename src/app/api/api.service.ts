...
const USER_API_HOST = environment.userApiHost;
...
export class ApiService {
  ...
  postUsers(endpoint, data): Promise<any> {
    const url = `${USER_API_HOST}${endpoint}`;
  ...
  async upload(endpoint: string, file: File, payload: any): Promise<any> {
    const signed_url = (await this.getFeed(`${endpoint}/signed-url/${file.name}`)).url;
    ...
    return new Promise ( resolve => {
        ...
        if (resp && (<any> resp).status && (<any> resp).status === 200) {
          resolve(this.postFeed(endpoint, payload));
        }
      });
    });
  }
  ...
  postFeed(endpoint, data): Promise<any> {
    const url = `${FEED_API_HOST}${endpoint}`;
    return this.http.post<HttpEvent<any>>(url, data, this.httpOptions)
            .toPromise()
            .catch((e) => {
